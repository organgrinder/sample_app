...
# it { @user.remember_token.should_not be_blank }
  ...
  describe "micropost association" do
    ...
    it "user death should destroy associated microposts" do
    ...
    describe "status" do
      ...
      before do
        ...
        3.times { followed_user.microposts.create!(content: "Hello bastards") }
      end
      
      its (:feed) { should include(newer_micropost) }
      its (:feed) { should include(older_micropost) }
      its (:feed) { should_not include(unfollowed_post) }
# include is a special way (available in rspec) of accessing include? method, 
# which checks if an array includes the given element
...
# this accesses the 'following?' method that we wrote in user.rb
    
    its(:followed_users) { should include(other_user) }
# this ':' syntax is surprising
